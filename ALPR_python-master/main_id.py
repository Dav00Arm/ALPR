    ...
    for frames in zip(*video_captures):
        ...
        for j in range(len(camera_urls)):
            ...
            for k,bbbox2 in enumerate(bbbox1):
                ...
                cv2.line(ill_frames[j],bbbox2[1],bbbox2[2],(255,0,84),thickness=2)
                cv2.line(ill_frames[j],bbbox2[0],bbbox2[1],(255,0,84),thickness=2)
                cv2.line(ill_frames[j],bbbox2[2],bbbox2[3],(255,0,84),thickness=2)
                cv2.line(ill_frames[j],bbbox2[3],bbbox2[0],(255,0,84),thickness=2)
            ...
            for key, value in current_spot_dict.items():
                ...
                if len(changed) > 0:
                    if changed[f'{cam_id}_{key}'] == 'Busy':
                        cv2.line(ill_frames[cam_id],bbboxes[cam_id][key][1],bbboxes[cam_id][key][2],(0,0,255),thickness=2)
                        cv2.line(ill_frames[cam_id],bbboxes[cam_id][key][0],bbboxes[cam_id][key][1],(0,0,255),thickness=2)
                        cv2.line(ill_frames[cam_id],bbboxes[cam_id][key][2],bbboxes[cam_id][key][3],(0,0,255),thickness=2)
                        cv2.line(ill_frames[cam_id],bbboxes[cam_id][key][3],bbboxes[cam_id][key][0],(0,0,255),thickness=2)
