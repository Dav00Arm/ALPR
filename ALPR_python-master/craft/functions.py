...
import numpy as np
import craft.utils
import craft.normalize
from configs.model_configs import craft_configs, refiner_configs


def craft_preprocess(image,canvas_size=100):
    img_resized, ratio_h,ratio_w = craft.normalize.resize_aspect_ratio(image, canvas_size, interpolation=cv2.INTER_LINEAR, mag_ratio=1)
    img_resized = cv2.resize(image,( canvas_size, canvas_size),cv2.INTER_AREA)
    ratio_h = 1 / ratio_h
    ratio_w = 1 / ratio_w

    # preprocessing
    x = craft.normalize.normalizeMeanVariance(img_resized)
...
def get_boxes(score_text, score_link, text_threshold, link_threshold, low_text, poly, ratio_w, ratio_h):
    boxes, polys = craft.utils.getDetBoxes(score_text, score_link, text_threshold, link_threshold, low_text, poly)
...
def predict_craft(image,
                  ...
                  canvas_size=100,
                  mode='onnx',
                  params=craft.utils.params_craft_refiner(craft_configs['craft_model'], refiner_configs['refine_net_model'])):
    """
    CRAFT inference
    """
