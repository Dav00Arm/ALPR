...
def predict_craft(image,
                ...
                low_text=low_text, 
                poly=poly, 
                refine_net=refine_net_model, 
                canvas_size=100, 
                mode='onnx',
                params=params_craft_refiner(craft_model,refine_net_model)):
    '''
    CRAFT inference
    '''
    ...
    if mode == 'gpu': 
        ...
        y, feature = net.run([params['craft'][1],params['craft'][2]], {params['craft'][0]: np.array(x).astype(np.float32)})
        y_refiner = refine_net.run([params['refine'][2]],{params['refine'][0]: np.array(y), params['refine'][1]: np.array(feature)})
