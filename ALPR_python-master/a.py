...
# # plate_recognition_model = torch.jit.load('./traced_ocr_model_fixed.pt').to(device)
# AlignCollate_demo = MyAlignCollate(imgH=imgH, imgW=imgW, keep_ratio_with_pad=False)
# plate_recognition_model.eval()

model = torch.nn.DataParallel(Model()).to(device)
# model.load_state_dict(torch.load(io.BytesIO(decrypt_file(load_graph(plate_recognition_weights), _key)), map_location=device)) 
model.load_state_dict(torch.load(plate_recognition_weights, map_location=device))
# # example = cv2.imread("/home/user/deep-text-recognition-benchmark/demo_image/demo_1.png").to(device)
example = torch.rand((1,1,32,100))
model.eval()
# example = example[0][0].fill_(0.2)
# example = example[0][1].fill_(0.2)

# text_for_pred = torch.LongTensor(1, 12).fill_(0).to(device)
